...
const FeedbackButton: React.FC<FeedbackButtonProps> = ({ lang }) => {
  ...
  return (
    <button
      ...
      className="fixed right-6 top-1/2 transform -translate-y-1/2 bg-[#F54963] hover:bg-[#D43A50] text-white p-3 md:p-4 rounded-full shadow-lg focus:outline-none focus:ring-2 focus:ring-[#F54963] focus:ring-opacity-75 transition-all duration-150 ease-in-out group z-50" /* LLYC Red */
    ...
    >
      ...
      <span className="absolute right-full top-1/2 transform -translate-y-1/2 mr-2 w-max bg-[#0A263B] text-xs text-white rounded-md px-2 py-1 opacity-0 group-hover:opacity-100 transition-opacity duration-150 pointer-events-none font-['Open_Sans']"> {/* LLYC Dark Blue tooltip */}
